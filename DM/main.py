...
df.to_csv('vaccine-type-ratio-district.csv')
...
state.to_csv('vaccine-type-ratio-state.csv')
...
pd.DataFrame([{'vaccineratio': ratio}]).to_csv('vaccine-type-ratio-overall.csv')
...
my_df = df.copy()
my_df = my_df[['District_Key','District', 'Dose1', 'Dose2' ]]
#my_df['Date'] = pd.to_datetime(my_df['Date'], format='%d/%m/%Y')
...
my_df.to_csv('vaccinated-dose-ratio-district.csv')
...
my_df = df.copy()
my_df = my_df[['State','State_Code', 'Dose1', 'Dose2' ]]
#my_df['Date'] = pd.to_datetime(my_df['Date'], format='%d/%m/%Y')
...
my_df.to_csv('vaccinated-dose-ratio-state.csv')
...
d1 = overall['Dose1'].sum()
d2 = overall['Dose2'].sum()

t =  overall['Total'].sum()
vaccinateddose1ratio = d1/t
vaccinateddose2ratio = d2/t

pd.DataFrame([{'vaccinateddose1ratio':vaccinateddose1ratio, 'vaccinateddose2ratio':vaccinateddose2ratio}]). \
to_csv('vaccinated-dose-ratio-overall.csv')
...
for state in all_states:
    ...
    tmp['rateofvaccination'] = (tdf.loc[0, 'Left'] - tdf.loc[l-1, 'Left'])/l
