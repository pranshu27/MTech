I have used Anaconda based Python interpreter, version 3.8.8 
...
2. Here we are using the file generated in the previous question to make an undirected graph which is then reported in the form of key value pairs. Since the graph is undirected, I have taken care that the entries like (a,b) and (b,a) do not occur simultaneously. Files created: edge_list.csv

3. The dataset used was 'https://data.covid19india.org/csv/latest/districts.csv'. Removal of cumulation in the data has been taken care of. Files created: month-cases-time.csv, week-cases-time.csv, overall-cases-time.csv

4. The dataset used for getting the cases was 'https://data.covid19india.org/csv/latest/districts.csv'. This file also had cumulative data which is removed using a method called shfit and subtract. The file 'https://data.covid19india.org/csv/latest/district_wise.csv' has been used for the district codes. Files created: districtid-peeks.csv, stateid-peeks.csv, overall-peeks.csv

5. The dataset used is 'http://data.covid19india.org/csv/latest/cowin_vaccine_data_districtwise.csv'.  Files created: monthid-districtid-vaccinated-count-time.csv, monthid-stateid-vaccinated-count-time.csv, overall-districtid-vaccinated-count-time.csv, overall-stateid-vaccinated-count-time.csv, weekid-districtid-vaccinated-count-time.csv, weekid-stateid-vaccinated-count-time.csv

6. The data for population has been taken from 'http://censusindia.gov.in/pca/DDW_PCA0000_2011_Indiastatedist.xlsx'. For vaccination data, 'http://data.covid19india.org/csv/latest/cowin_vaccine_data_districtwise.csv' is used and the final sets of ratios are calculated. Files created: districtid-vaccination-population-ratio.csv, overall-vaccination-population-ratio.csv, stateid-vaccination-population-ratio.csv

7. Dataset used is same as the previous one. Here we are calculating the ratios of different types of vaccines administered in India. The places where Covaxin is not available, we get the ratio as 'inf'. Files created: districtid-vaccine-type-ratio.csv, stateid-vaccine-type-ratio.csv, overall-vaccine-type-ratio.csv. 

8.Input datasets: http://data.covid19india.org/csv/latest/cowin_vaccine_data_districtwise.csv gives the number of people vaccinated, http://censusindia.gov.in/pca/DDW_PCA0000_2011_Indiastatedist.xlsx gives the population numbers on the basis of district/state. Files created: districtid-vaccinated-dose-ratio.csv, stateid-vaccinated-dose-ratio.csv, overall-vaccinated-dose-ratio.csv
